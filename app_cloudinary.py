...
def connect_to_sheets():
    ...
    try:
        ...
        # Устанавливаем числовой формат для столбца с суммами (столбец C)
        # Это нужно сделать только один раз, но безопасно делать каждый раз
        set_number_format(sheet)
...
def set_number_format(sheet):
    """Устанавливает числовой формат для ячеек с суммами"""
    try:
        # Получаем все значения, чтобы найти последнюю строку
        all_values = sheet.get_all_values()
        if not all_values:
            return
            
        # Определяем диапазон ячеек для столбца C (суммы)
        # Начинаем со второй строки (первая - заголовки)
        last_row = len(all_values)
        
        if last_row > 1:  # Если есть данные кроме заголовков
            # Форматируем диапазон C2:C{last_row} как числа
            range_str = f"C2:C{last_row}"
            
            # Устанавливаем числовой формат с запятой как десятичным разделителем
            # и пробелом как разделителем тысяч
            ...
            logger.info(f"Установлен числовой формат для диапазона {range_str}")
            
    except Exception as e:
        logger.warning(f"Не удалось установить числовой формат: {e}")
...
def handle_text(message):
    ...
    try:
        ...
        try:
            # Устанавливаем числовой формат для только что записанной ячейки
            sheet.format(f"C{next_row}", {
                "numberFormat": {
                    "type": "NUMBER",
                    "pattern": "#,##0.00"
                }
            ...
            logger.warning(f"Не удалось установить формат для ячейки C{next_row}: {e}")
    ...
    try:
        ...
        if sheet:
            try:
                ...
                # Форматируем ячейку с нулем как число
                try:
                    sheet.format(f"C{next_row}", {
                        "numberFormat": {
                            "type": "NUMBER",
        ...
        # ОТПРАВЛЯЕМ ФИНАЛЬНЫЙ ОТВЕТ БЕЗ КЛАВИАТУРЫ В edit_message_text
        success_msg = f"""
